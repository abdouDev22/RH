...
<body>
    ...
    <div>
      ...
      <button id="filter-button1" class="btn"><span>justifié</span> </button>
      <button id="filter-button2" class="btn"><span>Non-justifié</span> </button>
    </div>
...
if ($nombreLignes > 0) {
    ...
    $e = $ligne["Description_Justificatif"];
        ...
        echo '
      <div class="item">
        <div class="item-content"><span class="no">No :</span><span class="no1"><input type="number" value="'.$i.'"></span>
          ...
          <a href="descriptionModifier.php?id='.$i.'" class="voire" style="--clr:rgb(2, 2, 198);"><span>Voir plus</span><i></i></a>
      </div>
      </div>';}}
      else {
...
<script>
const voire=document.querySelector(".voire")
voire.addEventListener('click',() =>{
  localStorage.setItem("retour",1)
})
</script>
...
<script async>
  ...
  filterButton1.addEventListener('click', function () {
    ...
    for (var i = 0; i < items.length; i++) {
      var title = items[i].getElement().querySelector('.statut1').textContent;
      if (title=='justifié') {
        items[i].getElement().setAttribute('data-category', 'justifié');
      } else {
        items[i].getElement().setAttribute('data-category', '');
      }
    }
    grid.filter('[data-category="justifié"]');
  });
