...
toggle.addEventListener('click', (e) => {
  ...
  toggle.classList.toggle('active');
      ...
      <center><h1>postulation effectuer</h1></center>
      <div class="grid">
      <?php
...
$req = mysqli_query($connect, "
    SELECT * 
    FROM offres
    JOIN postuler ON offres.id_offres = postuler.id_offre
    JOIN candidats ON candidats.id_candidats = postuler.id_candidats
    WHERE postuler.id_offre = '$id_offres' and postuler.id_candidats='$login' ");

$l = mysqli_fetch_assoc($req) ;
    $ss = $l['nom'];
    $sss = $l['id_candidats'];
    $a = $l['id_offres'];
    $b = $l['titre_offre'];
    $c = $l['date_publication'];
    $d = $l['date_postulation'];
    $e = $l['date_limite'];
    $x= $l['id_postuler'];
   echo'

    <form action="des_modify.php" method="POST" enctype="multipart/form-data">
        <div class="des">
        <a href="postulation.php"  class="retourne_p"><b>retour</b><i></i></a>
        <h1 class="titre"> '.$b.'</h1>
            <span class="datcre">Date de postulation</span><span class="datcre1"><input type="datime" value="'.$d.'" readonly></span>
            <span class="datb">Date de Début</span> <span class="datb1"><input type="date" name="date_d" value="'.$c.'" readonly></span>
            <span class="datl">Date de limite</span><span class="datl1"><input type="date" name="date_f" value="'.$e.'" readonly></span>
            <input type=""hidden name="id" value="'.$x.'">
            <div class="p1"><p2>cv</p2>
                    <input type="file" name="cv" class="cv">
                </div>
                <div class="p2"><p1>lettre</p1>
                <input type="file" name="let" class="let">
            </div>
