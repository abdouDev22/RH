...
if (isset($_SESSION['id_directeur'])) {
    $id_directeur = $_SESSION['id_directeur'];
...
<head>
    ...
    <link rel="stylesheet" href="assetes/dossierdirec.css">
    <link rel="stylesheet" href="assetes/comt.css">
...
<body>
<?php
if(isset($_GET['id'])&&isset($_GET['id_conge'])){
  $_SESSION['dossier_id']=$_GET['id'];
  $_SESSION['dossier_conge']=$_GET['id_conge'];
}
...
$id_congé= $_SESSION['dossier_id'];
$_SESSION['id_congé']=$id_congé;
$sql="SELECT * FROM employes  WHERE   id_employe=$id_congé ";
$result_groupe = $conn->query($sql ); 
if ($result_groupe->num_rows > 0) {
  $row = $result_groupe->fetch_assoc();
  $nom = $row['nom'];
  $prenom=$row['prenom'];
...
$query_clients = "SELECT * FROM demande_congé,type_conge where id_employe=$id_congé and demande_congé.id_type_conge = type_conge.id_type_conge ORDER BY id_congé DESC LIMIT 3" ;
$result_clients = $conn->query($query_clients);

$sql_Absence=  "SELECT * FROM absences, type_absences WHERE type_absences.id_type_absences=absences.id_type_absences and id_employe=$id_congé  ORDER BY id_absences DESC LIMIT 3  ";
$result_Absence = $conn->query($sql_Absence);

$sql_Sanction=  "SELECT * FROM sanctions, type_sanctions WHERE sanctions.id_type_sanctions = type_sanctions.id_type_sanctions and id_employe=$id_congé  ORDER BY id_sanctions DESC LIMIT 3";
$result_sanction = $conn->query($sql_Sanction);
?>
    <nav>
      <a href="index.php">
        <div class="logo">
  ...
  <div class="content">
    <span class="nom">Nom</span><sapn class="nom1"><input type="text" name="" id="" value="<?php echo $nom; ?>"></sapn>
    <span class="prenom">Prenom</span><span class="prenom1"><input type="text" name="" id="" value="<?php echo $prenom;?> "></span>
    <span class="date">Date de naissance</span><span class="date1"><input type="date" value="<?php echo $date_naissance;?>"></span>
    <span class="tel">Telephone</span><span class="tel1"><input type="number" name="" value="<?php echo $telephone;?>"></span>
    <span class="add">Address</span><span class="add1"><input type="text" value="<?php echo $adresse;?>"></span>
    <span class="email">Email</span><span class="email1"><input type="email" name="" id="" value="<?php echo $email;?>"></span>
  </div>
  <div class="desdosiier">
    <button class="a">Action</button>
    <div class="Avantage">Liste des demande de congé</div>
    <div class="grid1">
    <?php
      if ($result_clients->num_rows > 0) {
        ...
        <div class="item">
            <div class="item-content">
              <span class="no">No :</span> <span class="no1"><?php echo $row['id_congé']; ?></span>
              <span class="date">Date de debut :</span><span class="date1">2002/01/06</span>
              <span class="datef">Date de fin :</span><span class="datef1">2003/04/09</span>
            </div>
          </div>
          <?php
        }
    ...
    <div class="sanction">Liste des Absence</div>
    <div class="grid2">
    <?php
      if ($result_Absence->num_rows > 0) {
        while ($row = $result_Absence->fetch_assoc()) {
            ?> 
      <div class="item">
        <div class="item-content">
          <span class="no">No :</span> <span class="no1"><?php echo $row['id_absences']; ?></span>
          <span class="date">Date de debut :</span><span class="date1">2002/01/06</span>
          <span class="datef">Date de fin :</span><span class="datef1">2003/04/09</span>
        </div>
      </div>
  ...
  <div class="demande">Liste des sanction</div>
  <div class="grid3">
  <?php
      if ($result_sanction->num_rows > 0) {
        while ($row = $result_sanction->fetch_assoc()) {
            ?> 
    <div class="item">
            <div class="item-content">
              <span class="no">No :</span> <span class="no1"><?php echo $row['id_sanctions']; ?></span>
              <span class="date">Date de debut :</span><span class="date1">2002/01/06</span>
              <span class="datef">Date de fin :</span><span class="datef1">2003/04/09</span>
            </div>
          </div>
...
<span class="lien valider"><a href="traitement/validation.php?id_conge=<?php echo $_SESSION['dossier_conge']; ?>"><B>valider</B></a></span>
<span class="lien refuser"><a href="traitement/refuser.php?id_conge=<?php echo $_SESSION['dossier_conge']; ?>"><B>Refuser</B></a></span>
  </div>
  <div class="popup">
    ...
    <div class="gg">
      <a href="tous/demandeDeConge.php?id=<?php echo $id_congé; ?>" class="a w">Tous les demande conge</a>
      <a href="tous/Absence.php?id=<?php echo $id_congé; ?>" class="b w">Tous les Absence</a>
      <a href="tous/Sanction.php?id=<?php echo $id_congé; ?>" class="c w">Tous les Sanction</a>
      <span class="lien valider"><a href="traitement/validation.php?id_conge=<?php echo $_SESSION['dossier_conge']; ?>"><B>valider</B></a></span>
      <span class="lien refuser"><a href="traitement/refuser.php?id_conge=<?php echo $_SESSION['dossier_conge']; ?>"><B>Refuser</B></a></span>
    </div>
  </div>
  ...
  <script>
    ...
    document.querySelectorAll('.w').forEach(element => element.addEventListener('click', () => {
  localStorage.setItem('interface', 2);
}));
    if(localStorage.getItem('interface')==2){
 ...
 function initAddEventListenerPopup() {
   ...
   btnPartage = document.querySelector(".a")
 ...
 initAddEventListenerPopup();
  ...
  <script>
     var grid = new Muuri('.grid1', {
        ...
        window.addEventListener('load', resizeGrid1);
        var grid3 = new Muuri('.grid3', {
          dragEnabled: true,
          dragHandle: '.item-content', // Activer la poignée de glisser-déposer pour déplacer les éléments
          layout: {
            fillGaps: true,
        ...
        window.addEventListener('load', resizeGrid3);
  </script>
