...
if (isset($_SESSION['id_directeur'])) {
    $id_manager = $_SESSION['id_directeur'];
...
<body>
    ...
    <div class="desdosiier">
        ...
        <h1 class="titre">Historique d'Affectation </h1>
        <div class="nb"><?php
$serveur = "localhost";
$utilisateur = "root";
$motdepasse = "";
...
$resultat = mysqli_query($sum, "SELECT COUNT(*) AS total_afectation FROM affectation 
    WHERE id_employe = '$id_employe'");

$row = mysqli_fetch_assoc($resultat);

$totalAfectation = $row['total_afectation'];

echo $totalAfectation;
?>
</div>
        ...
        <div class="grid">
          ...
          $resultat = mysqli_query($sum, "SELECT DISTINCT* FROM affectation
              WHERE id_employe = '$id_employe'");

              
$row = mysqli_fetch_assoc($resultat);

$ancienAfecation = $row['ancien_departement'];
$nouveauAfectation = $row['nouveau_departement'];

          echo'
            <div class="item1">
                <div class="item1-content">
                  <span class="no"></span> <span class="no1">1230</span>
                  <span class="depA">Ancien Département :</span><span class="depA1">'.$ancienAfecation.'</span>
                  <span class="depS">Département suivant :</span><span class="depS1">'.$nouveauAfectation.'</span>
