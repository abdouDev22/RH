...
if (isset($_SESSION['id_directeur'])) {
    $id_employes = $_SESSION['id_directeur'];
...
<head>
    ...
    <link rel="stylesheet" href="../asset/dossier.css">
</head>
<body>
    <nav>
...
$sum = mysqli_connect($serveur, $utilisateur, $motdepasse, $base);
        ...
        echo'
        <a href="description.php?id='.$id_employe.'" class="retourne"><b>retour</b><i></i></a>';?>
...
?>
        ...
        <div class="Avantage">Avantage sociaux</div>
        <div class="grid1">
  ...
  $resultat = mysqli_query($sum, "SELECT * FROM employes 
    INNER JOIN avantages_sociaux  ON employes.id_employe = avantages_sociaux.id_employe 
    WHERE employes.id_employe = '$id_employe' ORDER BY avantages_sociaux.date_debut DESC");
  ...
  if ($nombreLignes > 0) {
      ...
      $h = $ligne["date_debut"];
      $d = $ligne["date_fin"];
      $e = $ligne["id_avantages_sociaux"];
      
          echo '
            <div class="item">
                <div class="item-content">
                  <span class="no">No :</span> <span class="no1">'.$e.'</span>
                  <span class="date">Date de debut :</span><span class="date1">'.$h.'</span>
                  <span class="datef">Date de fin :</span><span class="datef1">'.$d.'</span>
                </div>
              </div>
              ';}}?>
             
        </div>
        <div class="sanction">sanction</div>
        <div class="grid2">
  ...
  $resultat = mysqli_query($sum, "SELECT * FROM employes 
    INNER JOIN sanctions ON employes.id_employe = sanctions.id_employe 
    WHERE employes.id_employe = '$id_employe' ORDER BY sanctions.date_sanction DESC");
  ...
  if ($nombreLignes > 0) {
      ...
      $h = $ligne["date_sanction"];
      $d = $ligne["date_fin"];
      $e = $ligne["id_sanctions"];
      ...
      <div class="demande">demande de congé</div>
      <div class="grid3">
  ...
  $resultat = mysqli_query($sum, "SELECT * FROM employes 
    INNER JOIN demande_congé  ON employes.id_employe = demande_congé.id_employe 
    WHERE employes.id_employe = '$id_employe' ORDER BY demande_congé.date_début DESC");
  ...
  if ($nombreLignes > 0) {
      ...
      <script>
        ...
        const dossier=localStorage.getItem("dossier")
        const lien_retour = document.querySelector('.retourne')

        if ( dossier == 1) {
    lien_retour.href = 'Traitement.php';
    localStorage.setItem("dossier",0)
  }
  if(dossier==2){
    lien_retour.href = 'description.php?id='+ $id_employe;
    localStorage.setItem("dossier",0)
  }
