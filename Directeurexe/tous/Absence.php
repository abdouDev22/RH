...
$id_employe=$_SESSION['dossier_id'];

$sql_em=  "SELECT DISTINCT* FROM employes where id_employe='$id_employe'";
$result_em = $conn->query($sql_em);
$sql_Absence=  "SELECT DISTINCT* FROM absences, type_absences WHERE type_absences.id_type_absences=absences.id_type_absences and id_employe=$id_employe  ORDER BY id_absences DESC  ";
...
?>
    ...
    <div class="desdosiier1">
        
        <a href="../dosier.php" class="retourne"><b>retour</b><i></i></a>
        <h1 class="titre">Absence</h1>
        <div class="nb">Nom: <?php
        ...
        while ($row = $result_Absence->fetch_assoc()) {
            ...
            <div class="item1">
                <div class="item1-content">
                  ...
                  <span class="dateD">Date de debut</span><span class="dateD1"><?php echo $row['date_debut']; ?></span>
                  <span class="datef">Date de fin</span><span class="datef1"><?php echo $row['date_fin']; ?></span>
                </div>
              </div>
      ...
      ?>
      <script>
          alert("Employe sans Sanction");
          window.location.href = "../dosier.php";
      </script>
      <?php
  }
  ?>
      ...
      <script>
        ...
        var grid = new Muuri('.grid', {
          dragEnabled: true,
          dragHandle: '.item1-content', // Activer la poignée de glisser-déposer pour déplacer les éléments
          layout: {
            fillGaps: true,
            horizontal: false,
          },
